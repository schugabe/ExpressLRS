...
// void ICACHE_RAM_ATTR sendSetVTXchannel(uint8_t band, uint8_t channel) // not used and not working at the moment, but don't delete
            ...
            for (;;)
            {
                ...
                void ICACHE_RAM_ATTR CRSF::UARTwdt()
                {
                    if (millis() > (UARTwdtLastChecked + UARTwdtInterval))
                    {
                        ...
                        //Serial.print("Stack remaining of: xESP32uartTask: ");
                        //Serial.println(uxTaskGetStackHighWaterMark(xESP32uartTask));
                        //Serial.print("Stack remaining of: xESP32uartTask: ");
                        //Serial.println(uxTaskGetStackHighWaterMark(xESP32uartWDT));
